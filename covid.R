...
raw[,12:80] <- as.integer(unlist(raw[,12:80]))
daysums <- rep(0,69)
...
colnames(placelist)=c("City","State","Total cases as of 3/30")
...
ratioprev <- rep(0,100)
for (i in 1:100){
  ratio[i] <- rawbycase$placesum[i] / rawbycase$placesum[i-1]
}
